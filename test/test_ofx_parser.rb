...
class OfxParserTest < MiniTest::Unit::TestCase
  ...
  OFX_FILES = Dir.open(fixtures_dir).each_with_object({}) do |fn, h|
    next unless fn =~ /\.ofx\.sgml$/

    ofx = File.read(fixtures_dir + "/#{fn}")
    ofx.gsub!(/\r?\n/, "\r\n") # change line endings to \r\n

    h[fn.scan(/\w+/).first.to_sym] = ofx
    h
  end.freeze
  ...
  def test_single_bank_account
    ...
    assert_equal 123_409, acct.balance_in_pennies
    ...
    assert_equal(-11_111, transactions[1].amount_in_pennies)
    ...
    assert_equal 111_111, transactions[2].amount_in_pennies
  ...
  def test_multiple_bank_accounts
    ...
    assert_equal(-11_112, transactions[4].amount_in_pennies)
  ...
  def test_single_credit_card
    ...
    assert_equal 1_900_099, acct.remaining_credit_in_pennies
    assert_kind_of DateTime, acct.remaining_credit_date
    assert_equal '-1111.01', acct.balance
    assert_equal(-111_101, acct.balance_in_pennies)
    assert_kind_of DateTime, acct.balance_date
